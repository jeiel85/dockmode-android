...
Version Files: app/build.gradle.kts, build.gradle.kts, gradle/libs.versions.toml, AGENTS.md, CHANGELOG.md
Build/Test Commands: ./gradlew ktlintCheck detekt testDebugUnitTest assembleDebug bundleRelease
Release Trigger: tag push
CI System: GitHub Actions
Expected Assets: APK, AAB, mapping.txt, release notes ZIP
Package Name: io.jeiel85.dockmode
Source Package Path: app/src/main/java/io/jeiel85/dockmode
Min SDK: 26
Target SDK: latest stable Android SDK installed in CI
Primary Language: Kotlin
Primary UI: Jetpack Compose
Primary Platform: Android phone, tablet, foldable
Primary Locale: ko
Required Locale: en
Optional Locale: ja, zh
```

원칙적으로 이 파일은 공통 작업 규칙과 프로젝트 고유 정책을 함께 담습니다. 제품 요구사항은 `SPEC.md`, 기술 설계는 `TECH_SPEC.md`, 작업 목록은 `TASKS.md`, 의사결정은 `DECISIONS.md`에 기록합니다.

---

## 1.1 DockMode 프로젝트 고유 정책

이 프로젝트는 Android 충전 거치대용 시계·달력 대시보드 앱입니다. iPhone의 StandBy 경험에서 아이디어를 얻되, Apple의 명칭, UI, 아이콘, 애니메이션, 화면 구성을 그대로 복제하지 않습니다.

에이전트는 다음 제약을 반드시 지킵니다.

- 앱은 Android 네이티브 방식으로 구현합니다: Kotlin, Jetpack Compose, AndroidX, Gradle Kotlin DSL.
- 핵심 모드는 두 가지입니다: `StandbyActivity` 수동 실행 모드와 `StandbyDreamService` Android 스크린세이버 모드.
- 충전 감지는 `BatteryManager`와 `ACTION_BATTERY_CHANGED` 기반으로 구현합니다.
- 일정 표시는 Android Calendar Provider를 사용합니다. `READ_CALENDAR` 권한은 사용자가 일정 기능을 켤 때만 요청합니다.
- 화면 유지 기능은 Activity에서만 `FLAG_KEEP_SCREEN_ON` 또는 Compose의 keepScreenOn 처리로 적용합니다. Service에서 화면 유지 플래그를 직접 적용하지 않습니다.
- 충전 연결만으로 백그라운드에서 Activity를 강제 실행하는 기능은 구현하지 않습니다. Android 백그라운드 Activity 시작 제한을 우회하지 않습니다.
- 네트워크 권한, 날씨 API, 로그인, 계정 연동, 광고, 분석, crash reporting, remote config는 사용자가 별도 승인하기 전까지 추가하지 않습니다.
- 개인정보는 로컬 우선으로 처리합니다. 캘린더 이벤트 제목과 시간은 화면 표시 목적 외 저장하지 않는 것을 기본값으로 합니다.
- OLED 번인 위험을 줄이기 위해 장시간 표시 화면에는 저휘도 모드, 번인 방지 위치 이동, 정적 요소 최소화 정책을 적용합니다.
- 앱 이름 `DockMode`, 프로젝트 코드 `DKM-ANDROID`, 레포 아이디 `dockmode-android`, Android 애플리케이션 ID `io.jeiel85.dockmode`를 초기 기준값으로 사용합니다. Play Store 출시 전 상표, 중복 앱명, 스토어 정책 검토를 별도 작업으로 수행합니다.
