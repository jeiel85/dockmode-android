...
- 마크다운 미지원 — 줄바꿈만 됨, `**굵게**` 같은 문법은 그대로 노출됩니다.
- HTML 미지원 (BCP-47 언어 태그만 허용됩니다).
- 이모지·유니코드 OK.
- 항목 머리표는 `•` (불릿) 또는 `-` 권장.
...
2. **App bundles**에 GitHub Release의 `dockmode-vX.Y.Z-release.aab` 업로드
3. ⚠️ **같은 화면에서 `dockmode-vX.Y.Z-mapping.txt` 도 함께 업로드**
   (AAB 옆 ⋮ 메뉴 → "Upload deobfuscation file" — Play Console의 R8/ProGuard 경고 해소용)
4. **What's new in this release** 입력란에 `vX.Y.Z.txt` 내용을 통째로 복사 (언어 태그 포함)
...
- [ ] 새 권한이 늘었다면 스토어 등록정보 권한 섹션도 함께 갱신

## DockMode 추가 점검 항목

- `READ_CALENDAR` 사용 이유 설명이 스토어 등록정보와 일치하는지
- 광고/분석/crash reporting SDK 미포함이 실제 빌드와 일치하는지
- 앱명 `DockMode`의 상표/중복 검토가 끝났는지 (출시 전 필수)
